...
4. Obtain the project. First check whether the current directory already contains main.tex, metadata.tex, polyu-thesis.cls, and .latexmkrc. If it does, use this working copy and do not clone another one. Otherwise, ask me for the destination directory, install Git only with my approval if it is missing, clone https://github.com/jinggqu/PolyU-Thesis-LaTeX, and enter the cloned project. Do not overwrite an existing directory or discard local changes.
